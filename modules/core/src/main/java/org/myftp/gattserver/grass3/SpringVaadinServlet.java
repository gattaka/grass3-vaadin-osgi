...
public class SpringVaadinServlet extends VaadinServlet {
	...
	private static final long serialVersionUID = 6377084968072044957L;
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		if (servletConfig.getInitParameter(BEAN_NAME_PARAMETER) != null) {
			vaadinBeanName = servletConfig
					.getInitParameter(BEAN_NAME_PARAMETER);
		...
		service.addSessionInitListener(new SessionInitListener() {
			...
			public void sessionInit(SessionInitEvent event)
				...
				event.getSession().addUIProvider(
						new GrassUIProvider(vaadinBeanName));
			}
		});
